...
namespace BanHangBeautify.Authentication.JwtBearer
{
    public class JwtSecurityStampHandler : IJwtSecurityStampHandler, ITransientDependency
    {
        ...
        public async Task<bool> Validate(ClaimsPrincipal claimsPrincipal)
        {
            ...
            var securityStampKey = claimsPrincipal.Claims.FirstOrDefault(c => c.Type == SPAConsts.SecurityStampKey);
            if (securityStampKey == null)
            {
                return false;
            }

            var userIdentifierString = claimsPrincipal.Claims.First(c => c.Type == SPAConsts.UserIdentifier);
        ...
        {
            await _cacheManager.GetCache(SPAConsts.SecurityStampKey)
        ...
        {
            await _cacheManager.GetCache(SPAConsts.SecurityStampKey).RemoveAsync(GenerateCacheKey(tenantId, userId));
        }
        ...
        private async Task<bool> ValidateSecurityStampFromCache(UserIdentifier userIdentifier, string securityStamp)
        {
            var securityStampKey = await _cacheManager
                .GetCache(SPAConsts.SecurityStampKey)
