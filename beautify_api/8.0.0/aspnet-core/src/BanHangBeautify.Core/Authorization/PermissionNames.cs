namespace BanHangBeautify.Authorization
{
    public static class PermissionNames
    {


        public const string Pages_Branch = "Pages.Branch";
        public const string Pages_Branch_Create = "Pages.Branch.Create";

        public const string Pages_CongTy = "Pages.CongTy";
        public const string Pages_CongTy_Create = "Pages.CongTy.Create";
        public const string Pages_CongTy_Edit = "Pages.CongTy.Edit";
        public const string Pages_CongTy_Delete = "Pages.CongTy.Delete";

        public const string Pages_ChiNhanh = "Pages.ChiNhanh";
        public const string Pages_ChiNhanh_Create = "Pages.ChiNhanh.Create";
        public const string Pages_ChiNhanh_Edit = "Pages.ChiNhanh.Edit";
        public const string Pages_ChiNhanh_Delete = "Pages.ChiNhanh.Delete";
        public const string Pages_ChiNhanh_Export = "Pages.ChiNhanh.Export";

        // sms- brandname
        public const string Pages_Brandname = "Pages.Brandname";
        public const string Pages_Brandname_Create = "Pages.Brandname.Create";
        public const string Pages_Brandname_Edit = "Pages.Brandname.Edit";
        public const string Pages_Brandname_Delete = "Pages.Brandname.Delete";
        public const string Pages_Brandname_Export = "Pages.Brandname.Export";
        public const string Pages_Brandname_Unlock = "Pages.Brandname.Unlock";
        public const string Pages_Brandname_NopTien = "Pages.Brandname.NopTien";

        public const string Pages_Brandname_ChuyenTien = "Pages.Brandname.ChuyenTien";
        public const string Pages_Brandname_ChuyenTien_Create = "Pages.Brandname.ChuyenTien.Create";
        public const string Pages_Brandname_ChuyenTien_Edit = "Pages.Brandname.ChuyenTien.Edit";
        public const string Pages_Brandname_ChuyenTien_Delete = "Pages.Brandname.ChuyenTien.Delete";
        #region Dịch Vụ
        public const string Pages_DM_HangHoa = "Pages.DM_HangHoa";
        public const string Pages_DM_HangHoa_Create = "Pages.DM_HangHoa.Create";
        public const string Pages_DM_HangHoa_Edit = "Pages.DM_HangHoa.Edit";
        public const string Pages_DM_HangHoa_Delete = "Pages.DM_HangHoa.Delete";
        public const string Pages_DM_HangHoa_Export = "Pages.DM_HangHoa.Export";
        public const string Pages_DM_HangHoa_Import = "Pages.DM_HangHoa.Import";
        public const string Pages_DM_HangHoa_Restore = "Pages.DM_HangHoa.Restore";

        public const string Pages_DM_LoaiHangHoa = "Pages.DM_LoaiHangHoa";
        public const string Pages_DM_LoaiHangHoa_Create = "Pages.DM_LoaiHangHoa.Create";
        public const string Pages_DM_LoaiHangHoa_Edit = "Pages.DM_LoaiHangHoa.Edit";
        public const string Pages_DM_LoaiHangHoa_Delete = "Pages.DM_LoaiHangHoa.Delete";

        public const string Pages_DM_NhomHangHoa = "Pages.DM_NhomHangHoa";
        public const string Pages_DM_NhomHangHoa_Create = "Pages.DM_NhomHangHoa.Create";
        public const string Pages_DM_NhomHangHoa_Edit = "Pages.DM_NhomHangHoa.Edit";
        public const string Pages_DM_NhomHangHoa_Delete = "Pages.DM_NhomHangHoa.Delete";

        public const string Pages_DonViQuiDoi = "Pages.DonViQuiDoi";
        public const string Pages_DonViQuiDoi_Create = "Pages.DonViQuiDoi.Create";
        public const string Pages_DonViQuiDoi_Edit = "Pages.DonViQuiDoi.Edit";
        public const string Pages_DonViQuiDoi_Delete = "Pages.DonViQuiDoi.Delete";
        #endregion
        #region Đặt lịch
        public const string Pages_Booking = "Pages.Booking";
        public const string Pages_Booking_Create = "Pages.Booking.Create";
        public const string Pages_Booking_Edit = "Pages.Booking.Edit";
        public const string Pages_Booking_Delete = "Pages.Booking.Delete";


        #endregion

        #region Notifications
        public const string Notifications = "Pages.Notifications";
        public const string Notifications_Booking = "Pages.Notifications.Booking";
        public const string Notifications_Birthday = "Pages.Notifications.Birthday";
        #endregion

        #region Khách Hàng
        public const string Pages_LoaiKhach = "Pages.LoaiKhach";
        public const string Pages_LoaiKhach_Create = "Pages.LoaiKhach.Create";
        public const string Pages_LoaiKhach_Edit = "Pages.LoaiKhach.Edit";
        public const string Pages_LoaiKhach_Delete = "Pages.LoaiKhach.Delete";

        public const string Pages_NhomKhach = "Pages.NhomKhach";
        public const string Pages_NhomKhach_Create = "Pages.NhomKhach.Create";
        public const string Pages_NhomKhach_Edit = "Pages.NhomKhach.Edit";
        public const string Pages_NhomKhach_Delete = "Pages.NhomKhach.Delete";

        public const string Pages_NguonKhach = "Pages.NguonKhach";
        public const string Pages_NguonKhach_Create = "Pages.NguonKhach.Create";
        public const string Pages_NguonKhach_Edit = "Pages.NguonKhach.Edit";
        public const string Pages_NguonKhach_Delete = "Pages.NguonKhach.Delete";

        public const string Pages_KhachHang = "Pages.KhachHang";
        public const string Pages_KhachHang_Create = "Pages.KhachHang.Create";
        public const string Pages_KhachHang_Edit = "Pages.KhachHang.Edit";
        public const string Pages_KhachHang_Delete = "Pages.KhachHang.Delete";
        public const string Pages_KhachHang_Import = "Pages.KhachHang.Import";
        public const string Pages_KhachHang_Export = "Pages.KhachHang.Export";
        #endregion

        #region Nhân viên
        public const string Pages_NhanSu = "Pages.NhanSu";
        public const string Pages_NhanSu_Create = "Pages.NhanSu.Create";
        public const string Pages_NhanSu_Edit = "Pages.NhanSu.Edit";
        public const string Pages_NhanSu_Delete = "Pages.NhanSu.Delete";
        public const string Pages_NhanSu_Import = "Pages.NhanSu.Import";
        public const string Pages_NhanSu_Export = "Pages.NhanSu.Export";

        public const string Pages_NhanVien_DichVu = "Pages.NhanVienDichVu";
        public const string Pages_NhanVien_DichVu_Create = "Pages.NhanVienDichVu.Create";
        public const string Pages_NhanVien_DichVu_Edit = "Pages.NhanVienDichVu.Edit";
        public const string Pages_NhanVien_DichVu_Delete = "Pages.NhanVienDichVu.Delete";

        public const string Pages_CaLamViec = "Pages.CaLamViec";
        public const string Pages_CaLamViec_Create = "Pages.CaLamViec.Create";
        public const string Pages_CaLamViec_Edit = "Pages.CaLamViec.Edit";
        public const string Pages_CaLamViec_Delete = "Pages.CaLamViec.Delete";
        public const string Pages_CaLamViec_Import = "Pages.CaLamViec.Import";
        public const string Pages_CaLamViec_Export = "Pages.CaLamViec.Export";

        public const string Pages_QuaTrinhCongTac = "Pages.QuaTrinhCongTac";
        public const string Pages_QuaTrinhCongTac_Create = "Pages.QuaTrinhCongTac.Create";
        public const string Pages_QuaTrinhCongTac_Edit = "Pages.QuaTrinhCongTac.Edit";
        public const string Pages_QuaTrinhCongTac_Delete = "Pages.QuaTrinhCongTac.Delete";

        public const string Pages_PhongBan = "Pages.PhongBan";
        public const string Pages_PhongBan_Create = "Pages.PhongBan.Create";
        public const string Pages_PhongBan_Edit = "Pages.PhongBan.Edit";
        public const string Pages_PhongBan_Delete = "Pages.PhongBan.Delete";

        public const string Pages_ChucVu = "Pages.ChucVu";
        public const string Pages_ChucVu_Create = "Pages.ChucVu.Create";
        public const string Pages_ChucVu_Edit = "Pages.ChucVu.Edit";
        public const string Pages_ChucVu_Delete = "Pages.ChucVu.Delete";

        public const string Pages_CauHinhPhanMem = "Pages.CauHinhPhanMem";
        public const string Pages_CauHinhPhanMem_Create = "Pages.CauHinhPhanMem.Create";
        public const string Pages_CauHinhPhanMem_Edit = "Pages.CauHinhPhanMem.Edit";
        public const string Pages_CauHinhPhanMem_Delete = "Pages.CauHinhPhanMem.Delete";

        public const string Pages_MauIn = "Pages.MauIn";
        public const string Pages_MauIn_Create = "Pages.MauIn.Create";
        public const string Pages_MauIn_Edit = "Pages.MauIn.Edit";
        public const string Pages_MauIn_Delete = "Pages.MauIn.Delete";

        public const string Pages_NganHang = "Pages.NganHang";
        public const string Pages_NganHang_Create = "Pages.NganHang.Create";
        public const string Pages_NganHang_Update = "Pages.NganHang.Update";
        public const string Pages_NganHang_Delete = "Pages.NganHang.Delete";

        public const string Pages_TaiKhoanNganHang = "Pages.TaiKhoanNganHang";
        public const string Pages_TaiKhoanNganHang_Create = "Pages.TaiKhoanNganHang.Create";
        public const string Pages_TaiKhoanNganHang_Edit = "Pages.TaiKhoanNganHang.Edit";
        public const string Pages_TaiKhoanNganHang_Delete = "Pages.TaiKhoanNganHang.Delete";

        public const string Pages_Phong = "Pages.Phong";
        public const string Pages_Phong_Create = "Pages.Phong.Create";
        public const string Pages_Phong_Edit = "Pages.Phong.Edit";
        public const string Pages_Phong_Delete = "Pages.Phong.Delete";

        public const string Pages_HeThongSMS = "Pages.HeThongSMS";
        public const string Pages_HeThongSMS_Create = "Pages.HeThongSMS.Create";
        public const string Pages_HeThongSMS_Edit = "Pages.HeThongSMS.Edit";
        public const string Pages_HeThongSMS_Delete = "Pages.HeThongSMS.Delete";
        public const string Pages_HeThongSMS_Resend = "Pages.HeThongSMS.Resend";

        public const string Pages_SMS_Template = "Pages.SMS_Template";
        public const string Pages_SMS_Template_Create = "Pages.SMS_Template.Create";
        public const string Pages_SMS_Template_Edit = "Pages.SMS_Template.Edit";
        public const string Pages_SMS_Template_Delete = "Pages.SMS_Template.Delete";

        public const string Pages_ViTriPhong = "Pages.ViTriPhong";
        public const string Pages_ViTriPhong_Create = "Pages.ViTriPhong.Create";
        public const string Pages_ViTriPhong_Edit = "Pages.ViTriPhong.Edit";
        public const string Pages_ViTriPhong_Delete = "Pages.ViTriPhong.Delete";

        public const string Pages_Booking_Color = "Pages.Booking_Color";

        public const string Pages_CauHinhChungTu = "Pages_CauHinhChungTu";
        public const string Pages_CauHinhChungTu_Create = "Pages_CauHinhChungTu.Create";
        public const string Pages_CauHinhChungTu_Edit = "Pages_CauHinhChungTu.Edit";
        public const string Pages_CauHinhChungTu_Delete = "Pages_CauHinhChungTu.Delete";

        public const string Pages_CauHinhTichDiem = "Pages_CauHinhTichDiem";
        public const string Pages_CauHinhTichDiem_Create = "Pages_CauHinhTichDiem.Create";
        public const string Pages_CauHinhTichDiem_Edit = "Pages_CauHinhTichDiem.Edit";
        public const string Pages_CauHinhTichDiem_Delete = "Pages_CauHinhTichDiem.Delete";

        public const string Pages_CheckIn = "Pages.CheckIn";
        public const string Pages_CheckIn_Create = "Pages.CheckIn.Create";
        public const string Pages_CheckIn_Edit = "Pages.CheckIn.Edit";
        public const string Pages_CheckIn_Delete = "Pages.CheckIn.Delete";

        public const string Pages_ChietKhauDichVu = "Pages.ChietKhauDichVu";
        public const string Pages_ChietKhauDichVu_Create = "Pages.ChietKhauDichVu.Create";
        public const string Pages_ChietKhauDichVu_Edit = "Pages.ChietKhauDichVu.Edit";
        public const string Pages_ChietKhauDichVu_Delete = "Pages.ChietKhauDichVu.Delete";
        public const string Pages_ChietKhauDichVu_Export = "Pages.ChietKhauDichVu.Export";

        public const string Pages_ChietKhauHoaDon = "Pages.ChietKhauHoaDon";
        public const string Pages_ChietKhauHoaDon_Create = "Pages.ChietKhauHoaDon.Create";
        public const string Pages_ChietKhauHoaDon_Edit = "Pages.ChietKhauHoaDon.Edit";
        public const string Pages_ChietKhauHoaDon_Delete = "Pages.ChietKhauHoaDon.Delete";

        public const string Pages_HoaDon_Anh = "Pages.HoaDon_Anh";
        public const string Pages_HoaDon_Anh_Create = "Pages.HoaDon_Anh.Create";
        public const string Pages_HoaDon_Anh_Update = "Pages.HoaDon_Anh.Edit";
        public const string Pages_HoaDon_Anh_Delete = "Pages.HoaDon_Anh.Delete";

        public const string Pages_HoaDon = "Pages.HoaDon";
        public const string Pages_HoaDon_Create = "Pages.HoaDon.Create";
        public const string Pages_HoaDon_Edit = "Pages.HoaDon.Edit";
        public const string Pages_HoaDon_Delete = "Pages.HoaDon.Delete";
        public const string Pages_HoaDon_Import = "Pages.HoaDon.Import";
        public const string Pages_HoaDon_Export = "Pages.HoaDon.Export";
        public const string Pages_HoaDon_Print = "Pages.HoaDon.Print";

        public const string Pages_LoaiChungTu = "Pages.LoaiChungTu";
        public const string Pages_LoaiChungTu_Create = "Pages.LoaiChungTu.Create";
        public const string Pages_LoaiChungTu_Edit = "Pages.LoaiChungTu.Edit";
        public const string Pages_LoaiChungTu_Delete = "Pages.LoaiChungTu.Delete";
        public const string Pages_LoaiChungTu_Import = "Pages.LoaiChungTu.Import";
        public const string Pages_LoaiChungTu_Export = "Pages.LoaiChungTu.Export";


        public const string Pages_NhanVienThucHien = "Pages.NhanVienThucHien";
        public const string Pages_NhanVienThucHien_Create = "Pages.NhanVienThucHien.Create";
        public const string Pages_NhanVienThucHien_Edit = "Pages.NhanVienThucHien.Edit";
        public const string Pages_NhanVienThucHien_Delete = "Pages.NhanVienThucHien.Delete";

        public const string Pages_NhomKhach_DieuKien = "Pages.NhomKhachDieuKien";
        public const string Pages_NhomKhach_DieuKien_Create = "Pages.NhomKhachDieuKien.Create";
        public const string Pages_NhomKhach_DieuKien_Edit = "Pages.NhomKhachDieuKien.Edit";
        public const string Pages_NhomKhach_DieuKien_Delete = "Pages.NhomKhachDieuKien.Delete";

        public const string Pages_KhuyenMai = "Pages.KhuyenMai";
        public const string Pages_KhuyenMai_Create = "Pages.KhuyenMai.Create";
        public const string Pages_KhuyenMai_Edit = "Pages.KhuyenMai.Edit";
        public const string Pages_KhuyenMai_Delete = "Pages.KhuyenMai.Delete";

        public const string Pages_KhuyenMai_ApDung = "Pages.KhuyenMaiApDung";
        public const string Pages_KhuyenMai_ApDung_Create = "Pages.KhuyenMaiApDung.Create";
        public const string Pages_KhuyenMai_ApDung_Edit = "Pages.KhuyenMaiApDung.Edit";
        public const string Pages_KhuyenMai_ApDung_Delete = "Pages.KhuyenMaiApDung.Delete";

        public const string Pages_NhanSu_LichLamViec = "Pages.NhanSu_CaLamViec";
        public const string Pages_NhanSu_LichLamViec_Create = "Pages.NhanSu_CaLamViec.Create";
        public const string Pages_NhanSu_LichLamViec_Edit = "Pages.NhanSu_CaLamViec.Edit";
        public const string Pages_NhanSu_LichLamViec_Delete = "Pages.NhanSu_CaLamViec.Delete";

        public const string Pages_NhanSu_LichLamViec_Ca = "Pages.NhanSu_LichLamViec_Ca";
        public const string Pages_NhanSu_LichLamViec_Ca_Create = "Pages.NhanSu_LichLamViec_Ca.Create";
        public const string Pages_NhanSu_LichLamViec_Ca_Edit = "Pages.NhanSu_LichLamViec_Ca.Edit";
        public const string Pages_NhanSu_LichLamViec_Ca_Delete = "Pages.NhanSu_LichLamViec_Ca.Delete";

        public const string Pages_NhanSu_NgayNghiLe = "Pages.NhanSu_NgayNghiLe";
        public const string Pages_NhanSu_NgayNghiLe_Create = "Pages.NhanSu_NgayNghiLe.Create";
        public const string Pages_NhanSu_NgayNghiLe_Edit = "Pages.NhanSu_NgayNghiLe.Edit";
        public const string Pages_NhanSu_NgayNghiLe_Delete = "Pages.NhanSu_NgayNghiLe.Delete";
        public const string Pages_NhanSu_NgayNghiLe_Import = "Pages.NhanSu_NgayNghiLe.Import";
        public const string Pages_NhanSu_NgayNghiLe_Export = "Pages.NhanSu_NgayNghiLe.Export";

        public const string Pages_NhanSu_TimeOff = "Pages.NhanSu_TimeOff";
        public const string Pages_NhanSu_TimeOff_Create = "Pages.NhanSu_TimeOff.Create";
        public const string Pages_NhanSu_TimeOff_Edit = "Pages.NhanSu_TimeOff.Edit";
        public const string Pages_NhanSu_TimeOff_Delete = "Pages.NhanSu_TimeOff.Delete";
        public const string Pages_NhanSu_TimeOff_Import = "Pages.NhanSu_TimeOff.Import";
        public const string Pages_NhanSu_TimeOff_Export = "Pages.NhanSu_TimeOff.Export";

        public const string Pages_NhatKyThaoTac = "Pages.NhatKyThaoTac";
        public const string Pages_NhatKyThaoTac_Create = "Pages.NhatKyThaoTac.Create";
        public const string Pages_NhatKyThaoTac_Edit = "Pages.NhatKyThaoTac.Edit";
        public const string Pages_NhatKyThaoTac_Delete = "Pages.NhatKyThaoTac.Delete";

        public const string Pages_QuyHoaDon = "Pages.QuyHoaDon";
        public const string Pages_QuyHoaDon_Create = "Pages.QuyHoaDon.Create";
        public const string Pages_QuyHoaDon_Edit = "Pages.QuyHoaDon.Edit";
        public const string Pages_QuyHoaDon_Delete = "Pages.QuyHoaDon.Delete";
        public const string Pages_QuyHoaDon_Export = "Pages.QuyHoaDon.Export";
        public const string Pages_QuyHoaDon_Print = "Pages.QuyHoaDon.Print";

        public const string Pages_KhoanThuChi = "Pages.KhoanThuChi";
        public const string Pages_KhoanThuChi_Create = "Pages.KhoanThuChi.Create";
        public const string Pages_KhoanThuChi_Edit = "Pages.KhoanThuChi.Edit";
        public const string Pages_KhoanThuChi_Delete = "Pages.KhoanThuChi.Delete";
        #endregion
        //COMMON PERMISSIONS (FOR BOTH OF TENANTS AND HOST)

        public const string Pages = "Pages";
        public const string Pages_Users_Activation = "Pages.Users.Activation";



        public const string Pages_Tenants = "Pages.Tenants";
        public const string Pages_Tenants_Create = "Pages.Tenants.Create";
        public const string Pages_Tenants_Edit = "Pages.Tenants.Edit";
        public const string Pages_Tenants_ChangeFeatures = "Pages.Tenants.ChangeFeatures";
        public const string Pages_Tenants_Delete = "Pages.Tenants.Delete";
        public const string Pages_Administration = "Pages.Administration";
        public const string Pages_Administration_Roles = "Pages.Administration.Roles";
        public const string Pages_Administration_Roles_Create = "Pages.Administration.Roles.Create";
        public const string Pages_Administration_Roles_Edit = "Pages.Administration.Roles.Edit";
        public const string Pages_Administration_Roles_Delete = "Pages.Administration.Roles.Delete";

        public const string Pages_Administration_Users = "Pages.Administration.Users";
        public const string Pages_Administration_Users_Create = "Pages.Administration.Users.Create";
        public const string Pages_Administration_Users_Edit = "Pages.Administration.Users.Edit";
        public const string Pages_Administration_Users_Delete = "Pages.Administration.Users.Delete";
        public const string Pages_Administration_Users_ChangePermissions = "Pages.Administration.Users.ChangePermissions";
        public const string Pages_Administration_Users_Impersonation = "Pages.Administration.Users.Impersonation";
        public const string Pages_Administration_Users_Unlock = "Pages.Administration.Users.Unlock";
        public const string Pages_Administration_AuditLogs = "Pages.Administration.AuditLogs";
    }
}
