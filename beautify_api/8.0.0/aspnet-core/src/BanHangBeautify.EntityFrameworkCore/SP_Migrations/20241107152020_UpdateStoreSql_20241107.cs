...
{
    /// <inheritdoc />
    public partial class UpdateStoreSql20241107 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"ALTER PROCEDURE [dbo].[GetNhatKyHoatDong_ofKhachHang]
	...
	(
		...
		union all
		  ...
		  select 
			...
			case hd.IdLoaiChungTu
				when 16 then concat(N'Điều chỉnh số dư thẻ với giá trị ', format(hd.TongTienHang, '#,###'))
				when 1 then concat(N'Mua hóa đơn với giá trị ', format(hd.TongTienHang, '#,###'))
				when 2 then concat(N'Mua gói dịch vụ với giá trị ', format(hd.TongTienHang, '#,###'))
				when 8 then concat(N'Nạp thẻ với giá trị ', format(hd.TongTienHang, '#,###'))
				when 6 then concat(N'Khách trả hàng với giá trị ', format(hd.TongTienHang, '#,###'))
			else '' end as HoatDong
		from BH_HoaDon hd
...
END");
            ...
            migrationBuilder.Sql(@"ALTER PROCEDURE [dbo].[GetCustomerDetail_FullInfor]
	@IdKhachHang uniqueidentifier
AS
BEGIN
...
BEGIN
	...
	----- get hd into temp table ---
		...
		and hd.IdLoaiChungTu not in (16) --- dieuchinh TGT --
					...
					FROM
						(SELECT 
							...
							LEFT JOIN (
								...
								FROM BH_HoaDon
								WHERE IsDeleted = 0 and IdLoaiChungTu not in (3,16) --- 3.baogia, 16.dieuchinh sodu
								GROUP BY IdKhachHang
